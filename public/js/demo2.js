 ...
 */
{
  ...
  imagesLoaded(document.querySelectorAll('.slide__img'), {background: true}, () => {
        ...
        for (const slide of slides) {
            ...
            const slideshow = new GlitchSlideshow(slide);
            // const nav = Array.from(document.querySelectorAll('.slide-nav__button'));
            const nav = Array.from(slide.querySelectorAll('.slide-nav__button'));
            // nav[0].addEventListener('click', () => slideshow.navigate('prev'));
            // nav[1].addEventListener('click', () => slideshow.navigate('next'));
            for (let i = 0; i < nav.length; i++) {
                if (i % 2 == 0) {
                    nav[i].addEventListener('click', () => slideshow.navigate('prev'));
                } else {
                    nav[i].addEventListener('click', () => slideshow.navigate('next'));
                }
            }
        }
    });
